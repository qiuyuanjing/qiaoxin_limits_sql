           ...
           BEGIN
              ...
              IF myflag>0 THEN
               ...
               IF tempList IS NOT NULL THEN 
                  UPDATE DEV1_OCS.DOCMETA@to_ecm_dev a SET a.xclbrauserlist = a.xclbrauserlist || ',&&' || upper(userList) || '(RWDA)' WHERE a.did = my_docm_did;
               ELSE
                  UPDATE DEV1_OCS.DOCMETA@to_ecm_dev a SET a.xclbrauserlist = '&&' || upper(userList) || '(RWDA)' WHERE a.did = my_docm_did;
               END IF;
               RESULT := RESULT + 1;
              END IF;
           END;
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      UPDATE DEV1_OCS.DOCMETA@TO_ECM_DEV A SET A.XCLBRAUSERLIST = TEMPLIST WHERE A.DID = MY_DOCM_DID;
    END;
  END LOOP;
  CLOSE CUR_LIST;
  RETURN RESULT;
