...
--<招标>类型，编写获取权限名单の方法。
--drop function CUS_QYJ_FUNC_GETLIMITS_ZB
CREATE OR REPLACE FUNCTION CUS_QYJ_FUNC_GETLIMITS_HB(ZB_ID IN VARCHAR2,
                                                     XTYPE IN VARCHAR2,
                                                     HB_ID IN VARCHAR2)
  RETURN VARCHAR AS
  USER_NAME VARCHAR2(50);
...
BEGIN
  ...
  IF :NEW.SEALED_AUCTION_STATUS = 'UNLOCKED' THEN
    MYCOUNT := CUS_QYJ_FUNC_LIMITS_UPDATE_HB(:NEW.AUCTION_HEADER_ID);
  END IF;
END;

--回标发布时，添加供应商的权限到附件中的方法,ebs中添加,apps
--DROP FUNCTION  CUS_QYJ_FUNC_LIMITS_UPDATE_HB;
CREATE OR REPLACE FUNCTION CUS_QYJ_FUNC_LIMITS_UPDATE_HB(ZB_ID IN VARCHAR2)
  RETURN INTEGER AS
  MY_DOCM_DID VARCHAR2(50);
  XTYPE       VARCHAR2(50);
  MY_HB_ID    VARCHAR2(50);
  TEMPLIST    VARCHAR2(300);
  CURSOR CUR_LIST IS
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      RESULT   := RESULT + 1;
      TEMPLIST := CUS_QYJ_FUNC_GETLIMITS_HB(ZB_ID, XTYPE ,MY_HB_ID);
