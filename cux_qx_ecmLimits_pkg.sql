CREATE OR REPLACE PACKAGE CUX_QX_ECMLIMITS_PKG IS
  ...
  ***********************************/
  ...
  /**********************************
  *  通过webService给OA待办推送消息
  *  作者：qyj
  *  时间：2013-5-9
  ***********************************/
  FUNCTION CUS_FUNC_SEND_NOTIFICATION RETURN INTEGER;
  
  /**********************************
  *  调用oa的webservice中的方法，setBPMTodoDB,INSERT
  *  作者：qyj
  *  时间：2013-5-9
  ***********************************/
  FUNCTION CUS_FUNC_SETBPMTODODB(NTF_ID IN NUMBER) RETURN VARCHAR2;
  
   /**********************************
  *  调用oa的webservice中的方法，setBPMTodoDB,DELETE
  *  作者：qyj
  *  时间：2013-5-9
  ***********************************/
  FUNCTION CUS_FUNC_DELBPMTODODB(NTF_ID IN NUMBER) RETURN VARCHAR2;
  
  /*******************************************
  *  更新供应商的"网上投标"时,刷新用户的Email
  *  作者：qyj
  *  时间：2013-5-9
  ********************************************/
  FUNCTION CUS_FUNC_SUPPLIER_EMAIL(VID NUMBER) RETURN INTEGER;
...
FUNCTION CUS_FUNC_SEND_NOTIFICATION RETURN INTEGER AS
  MYGUID   APPS.CUS_QX_SEND_NOTIFICATION.GUID%TYPE;
  MYSTATUS APPS.CUS_QX_SEND_NOTIFICATION.SENDSTATUS%TYPE;
  myResult VARCHAR2(200) := '';
  CURSOR CUR_LIST IS
    SELECT N.GUID, N.STATUS
      FROM APPS.CUS_QX_SEND_NOTIFICATION N
     WHERE N.SENDSTATUS = 'PENDING'
       FOR UPDATE;
  RESULT INTEGER;
BEGIN
  RESULT := 0;
  ...
  LOOP
    FETCH CUR_LIST
      INTO MYGUID, MYSTATUS;
    EXIT WHEN CUR_LIST%NOTFOUND;
    BEGIN
      IF MYSTATUS = 'OPEN' THEN
        myResult := CUS_FUNC_SETBPMTODODB(MYGUID);
      ELSIF MYSTATUS = 'CLOSED' THEN
        myResult := CUS_FUNC_DELBPMTODODB(MYGUID);
      END IF;
         UPDATE APPS.CUS_QX_SEND_NOTIFICATION N SET N.SENDSTATUS = 'FINISH' WHERE CURRENT OF CUR_LIST;
         RESULT := RESULT + 1;
    END;
  END LOOP;
  CLOSE CUR_LIST;
...
FUNCTION CUS_FUNC_SETBPMTODODB(NTF_ID IN NUMBER) RETURN VARCHAR2 AS
  ...
  P_TITLE          VARCHAR2(4000) := '';
  P_USERID         VARCHAR2(200) := '';
  P_AUTHORID       VARCHAR2(200) := '';
  P_CREATEDATE     VARCHAR2(200) := '';
BEGIN

  SELECT A.SUBJECT,
         lower(A.FROM_ROLE),
         lower(A.RECIPIENT_ROLE),
         TO_CHAR(A.BEGIN_DATE, 'yyyy-MM-dd hh:mm:ss')
    INTO P_TITLE, P_AUTHORID, P_USERID, P_CREATEDATE
    FROM APPS.WF_NOTIFICATIONS A
   WHERE A.NOTIFICATION_ID = NTF_ID;

  L_WSDL_URL       := 'http://portal.kingold.com/portal5.0/services/bpmService.jws?wsdl';
  L_SERVICE_NAME   := 'bpmService.jws';
  L_OPERATION_NAME := 'setBPMTodoDB';
  ...
  --******** 测试中 *********
    P_USERID   := 'chengyl';--'chenyh';--'chengyl';                                     
    P_AUTHORID := 'chengyl';--'chenyh';--'chengyl';
  --************************

  L_INPUT_PARAMS(0) := ANYDATA.CONVERTVARCHAR2('INSERT');
  L_INPUT_PARAMS(1) := ANYDATA.CONVERTVARCHAR2('EBS-' || NTF_ID);
  L_INPUT_PARAMS(2) := ANYDATA.CONVERTVARCHAR2('EBS');
  L_INPUT_PARAMS(3) := ANYDATA.CONVERTVARCHAR2(P_USERID);
  L_INPUT_PARAMS(4) := ANYDATA.CONVERTVARCHAR2(P_TITLE);
  L_INPUT_PARAMS(5) := ANYDATA.CONVERTVARCHAR2(P_AUTHORID);
  L_INPUT_PARAMS(6) := ANYDATA.CONVERTVARCHAR2(P_CREATEDATE);
  L_INPUT_PARAMS(7) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(8) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(9) := ANYDATA.CONVERTVARCHAR2('0');
  L_INPUT_PARAMS(10) := ANYDATA.CONVERTVARCHAR2('0');
  L_INPUT_PARAMS(11) := ANYDATA.CONVERTNUMBER(0);
...
EXCEPTION
  WHEN OTHERS THEN
    RETURN SUBSTR(SQLERRM, 0, 2000); --'error';
END;

FUNCTION CUS_FUNC_DELBPMTODODB(NTF_ID IN NUMBER) RETURN VARCHAR2 AS
  L_SERVICE        SYS.UTL_DBWS.SERVICE;
  L_CALL           SYS.UTL_DBWS.CALL;
  L_RESULT         ANYDATA;
  L_WSDL_URL       VARCHAR2(1024);
...
BEGIN
  ...
  L_INPUT_PARAMS(0) := ANYDATA.CONVERTVARCHAR2('DELETE');
  L_INPUT_PARAMS(1) := ANYDATA.CONVERTVARCHAR2('EBS-' || NTF_ID);
  L_INPUT_PARAMS(2) := ANYDATA.CONVERTVARCHAR2('EBS');
  L_INPUT_PARAMS(3) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(4) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(5) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(6) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(7) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(8) := ANYDATA.CONVERTVARCHAR2(' ');
  L_INPUT_PARAMS(9) := ANYDATA.CONVERTVARCHAR2('0');
  L_INPUT_PARAMS(10) := ANYDATA.CONVERTVARCHAR2('0');
...
FUNCTION CUS_FUNC_SUPPLIER_EMAIL(VID NUMBER,FLAG NUMBER) RETURN INTEGER AS
  MYUSER_ID NUMBER;
  CURSOR CUR_LIST IS
    SELECT FU.USER_ID AS USER_ID
     ...
     WHERE HP.PARTY_ID = HZR.SUBJECT_ID
       AND HZR.OBJECT_ID = HZR_HP.PARTY_ID
       ...
       AND HZR_HP.PARTY_NUMBER = PV.PARTY_NUMBER
       AND PV.VENDOR_ID = VID
    UNION
    SELECT FU.USER_ID AS USER_ID
      FROM HZ_PARTIES               HP,
           FND_USER                 FU,
     ...
     WHERE HP.PARTY_ID = HZR.SUBJECT_ID
       ...
       AND PV.VENDOR_ID = VID
    UNION
    SELECT FUU.USER_ID USER_ID
      FROM POS_CONTACT_REQUESTS  PCR,
           POS_SUPPLIER_MAPPINGS PSM,
           FND_USER              FUU,
           PO_VENDORS            PV
     WHERE PSM.MAPPING_ID = PCR.MAPPING_ID
       AND PSM.VENDOR_ID = PV.VENDOR_ID
       AND FUU.PERSON_PARTY_ID = PCR.CONTACT_PARTY_ID
       AND PV.VENDOR_ID = VID;
  RESULT INTEGER;
BEGIN
  RESULT := 0;
